	...
	if(check != ''){
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				ajaxGet(site_url+'document_management/document_management_client/bulk_delete_item?id='+check).done(function(success) {
					location.reload();
				}).fail(function(error) {
	...
	if(modal.find('input[name="action_type"]').val() == 'duplicate'){
		...
		if(select_folder != ''){
			if(selected_item != ''){
				...
				ajaxGet(site_url+'document_management/document_management_client/bulk_duplicate_item?selected_folder='+select_folder+'&selected_item='+selected_item).done(function(success) {
					location.reload();
				}).fail(function(error) {
	...
	if(modal.find('input[name="action_type"]').val() == 'move'){
		...
		if(select_folder != ''){
			if(selected_item != ''){
				...
				ajaxGet(site_url+'document_management/document_management_client/bulk_move_item?selected_folder='+select_folder+'&selected_item='+selected_item).done(function(success) {
					location.reload();
				}).fail(function(error) {
...
function show_select_folder(selected_folder, selected_id, action_type){
	...
	ajaxGet(site_url+'document_management/document_management_client/get_folder_list?parent=0&selected_folder='+selected_folder+'&selected_item='+selected_id).done(function (response) {
